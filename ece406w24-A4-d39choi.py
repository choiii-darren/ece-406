...
def random_adjacency_matrix(n):
    ...
    """
    LAMBDA = 0.9 # parameter for the Poisson distribution
...
def bellman_ford(A, s):
    ...
    def update(vertex_u, vertex_v, matrix_A):
        ...
        """
        assert np.abs(matrix_A[vertex_u.index, vertex_v.index]) > 0, '(u, v) must be a valid edge (non-zero length) in A'
        ################################################################################
        # TODO: implement update here
        if vertex_v.dist > vertex_u.dist + matrix_A[vertex_u.index, vertex_v.index]:
    ...
    for _ in range(n-1):
        for u in range(n):
            for v in range(n):
                if u != v and np.abs(A[vertices[u].index, vertices[v].index]) > 0:
                    update(vertices[u],vertices[v], A)
    
    oldVertices = [v.dist for v in vertices]
    print(oldVertices)
    for u in range(n):
        for v in range(n):
            if u != v and np.abs(A[vertices[u].index, vertices[v].index]) > 0:
...
def main():
    ...
    #
    s = 0 # index of tphe start vertex
